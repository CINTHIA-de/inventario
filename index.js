...
app.get('/editar', (req, res) => {
    const id = req.query.id
    let sql = 'select productos.id, nombre, productos.marca, precio, stock from productos, marcas where productos.marca = marcas.id and productos.id = ?'
...
app.get('/marcas', (req, res) => {
    ...
    base_datos.all(sql, (error, filas) => {
        if (error) {
            console.log('Error en la consulta BD')
        } else {
